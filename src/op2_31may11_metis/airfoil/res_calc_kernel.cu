...
/*
   int idx_arg6 = arg6_map*4;
   int idx_arg7 = arg7_map*4;
   int start_idx6;
   int start_idx7;
   for (int d=0; d<4; d++){
	   if( (threadIdx.x%4)==(idx_arg6+d)%4 )
             start_idx6=d;
	   if( (threadIdx.x%4)==(idx_arg7+d)%4 )
             start_idx7=d;
   }
   ...
   for (int col=0; col<ncolor; col++) {
      if (col2==col) {
        ...
        for (int d=0; d<4; d++){
            tmp_idx=(start_idx6+d) % 4;
            ind_arg3_s[idx_arg6 + tmp_idx] += arg6_l[tmp_idx];
            tmp_idx=(start_idx7+d) % 4;
            ind_arg3_s[idx_arg7 + tmp_idx] += arg7_l[tmp_idx];
         }
...
void op_par_loop_res_calc(char const *name, op_set set,                 
  ...
  // get plan                                                           
    ...
    int part_size = OP_part_size_res;	// For dynamically change by argv from main
  #endif                                                                
  ...
  for (int col=0; col < Plan->ncolors; col++) {                         
    ...
    //printf("col=%d, nblocks=%d, nthread=%d, nshared=%d\n", col, nblocks, nthread, nshared);
